...
class Board(Component):
    ...
    def render_welcome_info(self) -> None:
        st.write(
            f"Game {self.game.game_model.name} Ranking Table, {self.game.owner.username} is owner"
        )
        if self.game.owner._id == get_user_info().user_id:
            st.write("You are owner")
            r = row(2)
            if r.toggle("show token"):
                r.write(self.game.game_model.token)
        if st.button("Refresh"):
            st.rerun()

    def render_ranking_table(self) -> None:
        self.render_welcome_info()
        player_infos = [
            {
                "Name": name,
        ...
        st.data_editor(
            ...
            column_config={
                "Balance": st.column_config.ProgressColumn(
                    ...
                    else max([infos["Balance"] for infos in player_infos]),
                    min_value=0,
                ),
            },
    ...
    def render_transfer_form(self) -> None:
        ...
        self_index = next(
            (
                ...
                if player.user_model._id == user_id
            ),
            0,
        )
        ...
        from_player = st.selectbox(
            "From",
            self.game.players if not is_owner else self.game.players + [self.game.god],
        ...
        to_player = st.selectbox(
            "To",
            filter(lambda x: x.user_model._id != user_id or is_owner,self.game.players),
            format_func=lambda player: player.name,
        )
